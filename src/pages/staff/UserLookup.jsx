...
import { formatDate, formatDateTime, toDate } from '../../utils/formatters';
                  ...
                  {getUserAttendanceHistory(selectedUser.id).map((log, index) => (
                    <div key={index} className="bg-gray-50 rounded-lg p-3">
                      ...
                      <p className="text-sm text-gray-500 mt-1">
                        Duration: {(() => {
                          if (log.duration) return `${log.duration} min`;
                          const checkIn = toDate(log.checkInTime);
                          const checkOut = toDate(log.checkOutTime);
                          if (checkIn && checkOut) return `${Math.round((checkOut - checkIn) / 60000)} min`;
                          return 'In progress';
                        })()}
                      </p>
                    </div>
